...
deploy-airticket-goerli: check-env build
	forge script script/Airticket.s.sol:Deploy --rpc-url ${GOERLI_RPC_URL} --broadcast --verify -vvvv
...
ifndef GOERLI_PRIVATE_KEY
	$(error GOERLI_PRIVATE_KEY is undefined)
endif
ifndef GOERLI_RPC_URL
	$(error GOERLI_RPC_URL is undefined)
endif
